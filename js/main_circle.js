const maxRadius = 24, // maximum radius of circle
...
function bestCircleGenerator(maxRadius, padding) {
  ...
  return function(k) {
    ...
    for (var i = 0; i < k || bestDistance < padding; ++i) {
      ...
      var rad = 800 * Math.random();


      var x = 640 + Math.sin(angle) * (rad),
        y = 640 + Math.cos(angle) * (rad),
        rx1 = x - searchRadius,
        rx2 = x + searchRadius,
        ry1 = y - searchRadius,
        ry2 = y + searchRadius,
    ...
    var best = [bestX, bestY, bestDistance];
    quadtree.add(best);
    return best;
  };
}
